...
export default async function ProfilePage({
  ...
  return (
    <div className="mx-auto max-w-xl">
      <div className="flex items-start justify-between gap-4 border-b border-border px-4 py-6">
        <div className="min-w-0">
          ...
          {profile.bio && (
            <p className="mt-3 whitespace-pre-wrap break-words text-sm">{profile.bio}</p>
          )}
        ...
        <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full bg-surface text-xl font-semibold">
          {profile.avatar_url ? (
      ...
      {items.length === 0 ? (
        <p className="px-4 py-10 text-center text-sm text-muted">{t.profile.empty}</p>
      ) : (
        items.map((post) => (
